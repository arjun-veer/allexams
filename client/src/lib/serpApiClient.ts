...
export const checkForDuplicateExams = async (examName: string) => {
  try {
    ...
    const existingResponse = await apiClient.getExams({ 
      search: examName,
      limit: 5 
    }) as { success: boolean; data: { exams: Exam[] } };
    
    // Check pending exams
    const pendingResponse = await apiClient.getPendingExams({ 
      limit: 5 
    }) as { success: boolean; data: { pendingExams: any[] } };
    
    // Filter pending exams by name
    const pendingExams = pendingResponse.data.pendingExams?.filter(exam => 
      exam.name.toLowerCase().includes(examName.toLowerCase())
    ) || [];
    
    return {
      existingExams: existingResponse.data.exams || [],
