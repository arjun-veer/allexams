...
import { formatDateOptional } from "@/lib/dateUtils";
...
const ExamDetails = () => {
  // Route in App.tsx uses ":id"; align param name
  const { id: examId } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { exams, subscribeToExam, unsubscribeFromExam, fetchExams } = useExams();
  const [exam, setExam] = useState<Exam | null>(null);
  
  // Fetch exams from database if not already loaded
  useEffect(() => {
    if (exams.length === 0) {
      fetchExams();
    }
  ...
  useEffect(() => {
    ...
    if (foundExam) {
      ...
      // Only show error if exams are loaded but exam not found
      console.log('Exam not found, available exams:', exams.map(e => e.id));
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {isRegistrationActive() ? (
                ...
                <Badge variant="outline">
                  Registration starts {formatDateOptional(exam.registrationStartDate)}
                </Badge>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {formatDateOptional(exam.registrationStartDate)} - {formatDateOptional(exam.registrationEndDate)}
                    </p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {formatDateOptional(exam.examDate)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {formatDateOptional(exam.resultDate)}
                    </p>
                  </div>
                </div>
