...
export const ExamTracker = () => {
  const { mySubscriptions, exams, unsubscribeFromExam, fetchExams, fetchSubscriptions } = useExams();

  // Fetch exams and subscriptions from database on component mount
  useEffect(() => {
    fetchExams();
    fetchSubscriptions();
  }, [fetchExams, fetchSubscriptions]);

  // Debug logging
  console.log('ExamTracker Debug:');
  console.log('mySubscriptions:', mySubscriptions);
  console.log('exams:', exams);
  console.log('exams count:', exams.length);

  // Extract exam objects from subscriptions
  const subscribedExamObjects = mySubscriptions.map(subscription => subscription.exam);

  console.log('subscribedExamObjects:', subscribedExamObjects);
  console.log('subscribedExamObjects count:', subscribedExamObjects.length);
